...
const retryWithExponentialBackoff = async (
  ...
  try {
    ...
    if (retries > 0) {
      console.log(
        `Retrying operation. Attempts remaining: ${retries - 1}`
      );
      await sleep(delay);
  ...
  try {
    ...
    try {
      ...
      if (!resumeText || resumeText.trim().length === 0) {
        return res.status(400).json({
          error:
            "Could not extract text from file. File may be empty or corrupted.",
        });
      }
    } catch (fileProcessingError) {
      console.error("File processing error:", fileProcessingError);
...
1. ATS evaluation score breakdown out of 10 for:
- Keyword relevance and density
- Professional experience clarity
- Skills presentation
- Education formatting
...
2. Final ATS Score in this exact format:
ATS Score: X/100
...
`;
    ...
    try {
      ...
      const analysisText =
        await retryWithExponentialBackoff(generateAnalysis);
      ...
      if (scoreMatch) {
        ...
        // Fallback scoring
        const keywordMatches = {
          experience: resumeText.match(
            /experience|worked|managed|developed/gi
          ),
          skills: resumeText.match(
            /skills|proficient|expertise/gi
          ),
          education: resumeText.match(
            /education|degree|university/gi
          ),
          achievements: resumeText.match(
            /achieved|improved|increased/gi
          ),
          contact: resumeText.match(
            /email|phone|linkedin|github/gi
          ),
        };

        const matchCount = Object.values(keywordMatches).reduce(
          (total, matches) =>
        ...
        score = Math.min(
          Math.max(
            50 + matchCount * 5,
            Math.ceil(resumeText.length / 100)
          ),
          100
        );
      }
      ...
      if (
        !analysis.find((item) =>
          item.title.toLowerCase().includes("breakdown")
        )
      ) {
        const scoringFactors = [
          "Keyword Optimization",
        ...
        const breakdownSection = {
          title: "Scoring Breakdown",
          description: scoringFactors
            .map((factor) => {
              const factorScore = Math.floor(
                score / scoringFactors.length
              );
      ...
      return res.status(500).json({
        error: "Error generating ATS analysis",
        details:
